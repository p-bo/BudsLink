...
class ModeEditDialog extends Adw.Dialog {
    _init(modeRow, gtxt, modelData, mode) {
        super._init({
            title: mode.name,
            content_width: 360,
            content_height: 600,
        ...
        toolbarView.add_top_bar(new Adw.HeaderBar({show_end_title_buttons: true}));
        ...
        this._isSpatialPreset = modelData.audioModes.spatialPreset?.includes(presetKey);
        ...
        if (modelData.audioModes.spatialMode) {
            ...
            const presetOption = [_('Still'), _('Motion')];
            const customOption = [_('Off'), _('Still'), _('Motion')];
            const presetValues = [1, 2];
            const customValues = [0, 1, 2];

            this._spatialRow = new DropDownRowWidget({
                title: _('Spatial Audio'),
                options: this._isSpatialPreset ? presetOption : customOption,
                values: this._isSpatialPreset ? presetValues : customValues,
    ...
    updateUI() {
        ...
        if (this._spatialRow) {
            ...
            this._spatialRow.sensitive = (!this._isPreset || this._isSpatialPreset) &&
                (this._mode.flag & 4) !== 0;
        }
    }
...
const ModeRow = GObject.registerClass({
    ...
    _init(window, gtxt, modelData, mode, modes) {
        super._init();
        ...
        this.dialog = new ModeEditDialog(this, this._gtxt, modelData, this._mode);
...
export const ModesGroupWidget = GObject.registerClass({
    ...
    _init(window, gtxt, modelData, modes, currentMode) {
        super._init();
        this._window = window;
        this._gtxt = gtxt;
        const _ = gtxt;
